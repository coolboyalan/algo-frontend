export default TableContentManager = ({
  ...
  const [itemsPerPage, setItemsPerPage] = useState(10); // Defaulted to 10 for better view
  ...
  const [notification, setNotification] = useState({ type: "", message: "" }); // For CRUD operations

  const [isViewModalOpen, setIsViewModalOpen] = useState(false);
  const [isEditModalOpen, setIsEditModalOpen] = useState(false);
  const [isDeleteModalOpen, setIsDeleteModalOpen] = useState(false);
  const [selectedItem, setSelectedItem] = useState(null); // For view, edit, delete

  // Safer baseUrl initialization
  const baseUrl =
    (typeof process !== "undefined" &&
      process.env &&
      process.env.NEXT_PUBLIC_API_BASE_URL) ||
    (typeof window !== "undefined" && window.NEXT_PUBLIC_API_BASE_URL) ||
    ""; // Ensure baseUrl is defined, fallback to empty string
  ...
  const makeApiCall = async (endpoint, method = "GET", body = null) => {
    ...
    try {
      ...
      throw e; // Re-throw to be caught by specific handlers if needed
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      // Error already set by makeApiCall
      setData([]);
  ...
  const tableColumns = [
    ...initialColumns,
    {
      key: "actions",
      label: "Actions",
      type: "actions", // Custom type for rendering action buttons
      sortable: false, // Actions column is typically not sortable
    },
  ];
  ...
  const renderCell = (item, column) => {
    ...
    // Default rendering with improved object/array handling
    const displayValue =
  ...
  return (
    <DashboardLayout pageTitle={pageTitle}>
      ...
      >
        {(selectedItem || formFields.length > 0) && ( // Ensure formFields is available for create mode too if selectedItem is null
          <EditItemForm
            item={selectedItem}
            formFields={formFields}
            onSubmit={handleSaveEdit} // You'd also need a handleCreateSubmit
